...
const SINGLETON_ID = "homeContent-hero";
const MEDIA_TYPE = "heroMedia";
...
export default function HeroSection() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (doc) {
          setTitle(doc.heroTitle || "");
          setSubtitle(doc.heroSubtitle || "");
        }
        const items = await fetchMediaCollection(MEDIA_TYPE);
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const item = isVideo
          ? await addFileMedia(MEDIA_TYPE, file)
          : await addImageMedia(MEDIA_TYPE, file);
  ...
  return (
    <>
      <PageMeta title="Hero Section | Admin" description="Manage Hero Section" />
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
              <h3 className="text-base font-medium text-gray-800 dark:text-white/90">Images / Videos</h3>
              <button
            ...
            <div className="p-6">
              {media.length === 0 ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {media.map((item) => (
                    <div key={item.id} className="relative group rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 aspect-video bg-gray-100 dark:bg-gray-800">
                      ...
                      <span className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-xs px-2 py-1 truncate">
                        {item.mediaType === "video" ? "🎬" : "🖼"} {item.name}
                      </span>
                    </div>
                  ))}
                  <div
                    onClick={() => fileRef.current?.click()}
                    className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl aspect-video flex flex-col items-center justify-center cursor-pointer hover:border-brand-400 transition-colors"
                  >
