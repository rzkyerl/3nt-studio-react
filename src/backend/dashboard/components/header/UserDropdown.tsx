...
export default function UserDropdown() {
  ...
  function toggleDropdown() {
    ...
    setIsOpen(opening);
    if (opening) {
      window.dispatchEvent(new CustomEvent("dashboard:dropdown-open", { detail: "user" }));
    }
  }
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      if ((e as CustomEvent).detail !== "user") setIsOpen(false);
    };
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="block mr-1 font-medium text-theme-sm">{user?.displayName || "Admin"}</span>
        <svg
